...
type Config struct {
	...
	MaxRetries int
	Model      anthropic.Model // Default model for tests
}
...
func DefaultConfigWithPrefix(_ string) *Config {
	config := &Config{
		...
		BaseURL:    getEnvOrDefault("TEST_ANTHROPIC_BASE_URL", "http://localhost:8090/anthropic"),
		Timeout:    30 * time.Second,
		MaxRetries: 3,
		Model:      anthropic.Model(getEnvOrDefault("TEST_MODEL", "deepseek-chat")),
	}
...
func (c *Config) NewClient() anthropic.Client {
	if c.APIKey == "" {
...
// GetModel returns the configured model as a Model type
func (c *Config) GetModel() anthropic.Model {
	return c.Model
}
