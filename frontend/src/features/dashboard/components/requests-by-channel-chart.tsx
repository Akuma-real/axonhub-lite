...
export function RequestsByChannelChart({ timePeriod }: RequestsByChannelChartProps) {
  const { t, i18n } = useTranslation();
  const { data: channelData, isLoading: isRequestsLoading, isFetching: isRequestsFetching, error: requestsError } = useRequestsByChannel(timePeriod);
  const { data: costData, isLoading: isCostLoading, isFetching: isCostFetching, error: costError } = useCostByChannel(timePeriod);
  ...
  const { chartData, totalRequests, totalCost } = useMemo(() => {
    if (!channelData) return { chartData: [], totalRequests: 0, totalCost: 0 };

    const costMap = new Map((costData ?? []).map((item) => [item.channelName, item.cost]));

    const data = channelData
      .map((item) => ({
        name: item.channelName,
        requests: item.count,
        cost: costMap.get(item.channelName) ?? 0,
      }))
  ...
  const tooltipContent = (props: TooltipContentProps) => {
    ...
    const data = payload[0].payload as { name: string; requests: number; cost: number } | undefined;
    if (!data) return null;
